...
const lista = document.querySelector(".produtos__lista");
let html = "";
...
function constroiCard(imagem, nome, preco) {
    ...
    <div class="card">
        ...
        <div class="card-container--info">
            <p class="ibm-plex-mono-regular">${nome}</p>
            <div class="card-container--value">
                <p class="ibm-plex-mono-bold">R$ ${preco}</p>
                <svg class="card__excluir" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
async function listaCards() {
    try {
        ...
        if (listaApi.length > 1) {
            const metade = (Math.round(listaApi.length/2));
            const primeiraParte = listaApi.slice(0, metade);
            const segundaParte = listaApi.slice(metade);
            
            html += abreLinha();
            primeiraParte.forEach(card => html += constroiCard(card.imagem, card.nome, card.preco));
            html += fechaLinha();

            html += abreLinha();
            segundaParte.forEach(card => html += constroiCard(card.imagem, card.nome, card.preco));
            html += fechaLinha();

        } else {
            listaApi.forEach(card => html += constroiCard(card.imagem, card.nome, card.preco));
        }
        
    } catch {
        html = `<h2 class="mensagem__titulo">Não foi possível carregar a lista de produtos</h2>`;
